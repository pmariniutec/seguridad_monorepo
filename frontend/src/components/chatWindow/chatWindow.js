...
export default class ChatWindow extends Component {
  ...
  async componentDidMount() {
    try {
      let contactsResult = await API.getContacts(
        this.props.loggedInUserObj._id,
        this.props.loggedInUserObj.role
      );
    ...
    let localStorageChats = JSON.parse(
      localStorage.getItem(this.props.loggedInUserObj._id + "_messages")
    );
    this.setState({ chats: { ...localStorageChats } });

    let ws = new WebSocket(
      `ws://localhost:4000/chat/${this.props.loggedInUserObj._id}`
    );
    ...
    ws.onmessage = async (e) => {
      ...
      if (newMessage.senderid === this.props.loggedInUserObj._id) {
        ...
        let decrytedMessage =
          await this.props.signalProtocolManagerUser.decryptMessageAsync(
            newMessage.senderid,
            newMessage.message
          );
        ...
        this.setState(
          (prevState) => ({
            chats: {
              ...
              [newMessage.chatId]: {
                ...
                messages: [
                  ...prevState.chats[newMessage.chatId].messages.concat(
                    newMessage
                  ),
                ],
              },
          ...
          () =>
            localStorage.setItem(
              this.props.loggedInUserObj._id + "_messages",
        ...
        {this.state.messageToUser && (
          <MessageBox
            ...
            messages={
              ...
              this.state.chats[this.getSelectedUserChatId()].messages
            }
          />
        )}
      </div>
